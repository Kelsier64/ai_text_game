...
class Environment(Describable):
    ...
    def get_objects_in_view(self, location: Position) -> List[Dict[str, str]]:
        """Return objects and their positions relative to a character's location."""
        objects_in_view = []
        for obj in self.objects:
            angle = location.angle_to(obj.position)
            distance = location.distance_to(obj.position)
    ...
    def get_characters_in_view(self, location: Position) -> List[Dict[str, str]]:
        """Return characters and their positions relative to a character's location."""
        characters_in_view = []
        for character in self.characters:
            angle = location.angle_to(character.position)
            distance = location.distance_to(character.position)
...
class Character(Describable):
    ...
    def __init__(self, name: str, age: int, gender: str, environment: Environment,
                 position: Position, personality: str, mental_state: str, time_manager: TimeManager):
        self.name = name
        self.age = age
        self.gender = gender
        ...
        self.time_manager = time_manager  # Character now has a reference to time manager
    ...
    def get_description(self) -> str:
        return (f"Character: {self.name}, Age: {self.age}, Gender: {self.gender}, "
                f"Position: {self.position}, Doing: {self.doing}, Time: {self.time_manager.get_time()}")
...
# 創建角色
character = Character("Aragorn", 35, "Male", env1, Position(0, 0), "Brave", "Calm", time_manager)

# 角色與物品互動
character.interact_with_item(sword)

# 角色穿過門進入另一個環境
character.go_through_gate(gate)
