...
def str_request(messages, max_tokens):
    try:
        ...
        return "error"
def position(point1: Tuple[int, int], point2: Tuple[int, int]) -> Tuple[float, float]:
    x1, y1 = point1
    x2, y2 = point2
...
class Environment:
    def __init__(self, name: str, description: str, t: datetime):
        self.name = name
        ...
        self.roles: List[Character] = []

    def __str__(self) -> str:
        return f"Environment: {self.name}, Activity: {self.description}, Time: {self.time}, Date: {self.date}"


class Item:
    def __init__(self, name: str, location: Tuple[int, int], description: str):
        self.name = name
...
class Gate:
    def __init__(self, name: str, location: Tuple[int, int], description: str, connection: Tuple[Environment, Environment]):
        self.name = name
        self.location = location
        self.description = description
    ...
    def __str__(self) -> str:
        return (f"Gate: {self.name}, Description: {self.description}, "
                f"Connects {self.connection[0].name} and {self.connection[1].name}")


class Character:
    def __init__(self, name: str, age: int, gender: str, environment: Environment,
                 location: Tuple[int, int], position: str, personality: str, mental_state: str,
                 long_term_memory: List[str], life_memory: List[str], short_term_memory: List[str]):
        self.name = name
        self.age = age
        self.gender = gender
        ...
        self.external_conditions: Dict[str, str] = {}
        self.mood = "not set"
    ...
    def go(self, item: Item):
        """Move the character to the item's location."""
        self.position = item.name
        self.location = item.location

    def enter(self, gate: Gate):
        """Move the character between environments through a gate."""
        if self.environment == gate.connection[0]:
            self.environment = gate.connection[1]
        elif self.environment == gate.connection[1]:
            self.environment = gate.connection[0]

    def get_objects(self) -> List[Dict[str, str]]:
        """Get a list of objects in the character's environment, with their positions."""
        objects = []
    ...
    def get_people(self) -> List[Dict[str, str]]:
        """Get a list of people in the character's environment, with their positions."""
        people = []
        for person in self.environment.roles:
            angle, distance = position(self.location, person.location)
            people.append({
                "name": person.name,
                "description": person.doing,
                "position": f"{int(angle)} degrees / {int(distance)} meters"
            })
        return people

    def __str__(self) -> str:
        return f"Character: {self.name}, Position: {self.position}, Doing: {self.doing}"
